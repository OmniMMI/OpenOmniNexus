...
headers = {"User-Agent": "Open-Omni Client"}
...
def add_speech(state, speech, video, request: gr.Request):
    # text = "Please directly answer the questions in the user's speech."
    # text = '<speech>\n' + text
    
    video_text = '<image>\n'
    video_text = (video_text, video)
    speech_text = '<speech>\n'
    speech_text = (speech_text, speech)
    
    state = default_conversation.copy()
    state.append_message(state.roles[0], video_text)
...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens, chunk_size, request: gr.Request):
    ...
    if len(state.messages) == state.offset + 3:
        # First round of conversation
        template_name = "llava_llama_3" # modify by hand qwen_1_5
        template_name = "qwen_1_5"
        new_state = conv_templates[template_name].copy()
        new_state.append_message(new_state.roles[0], state.messages[-3][1])
    ...
    video = state.messages[0][1][1]
    sr, audio = state.messages[1][1][1]
...
title_markdown = ("""
# Open Omni: a fully open-source implementation of a GPT-4o-like speech-to-speech video understanding model
""")
...
def build_demo(embed_mode, vocoder, cur_dir=None, concurrency_count=10):
    with gr.Blocks(title="Open-Omni Chatbot", theme=gr.themes.Default(), css=block_css) as demo:
        state = gr.State()
        ...
        with gr.Row():
            video_input_box = gr.Video(sources=["upload", "webcam"], label="Video Input")
        ...
        gr.Examples(examples=[
            [f"{cur_dir}/assets/water.mp4"],
        ], inputs=[video_input_box])
        ...
        submit_btn.click(
            add_speech,
            [state, audio_input_box, video_input_box],
            [state]
        ).then(
            http_bot,
        ...
        clear_btn.click(
            clear_history,
            None,
            [state, audio_input_box, video_input_box, text_output_box, unit_output_box, audio_output_box],
            queue=False
        )
