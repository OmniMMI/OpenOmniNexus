...
from open_omni.constants import SPEECH_TOKEN_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_SPEECH_TOKEN
...
def load_video(video, max_frames_num=16):
...
class ModelWorker:
    def __init__(self, controller_addr, worker_addr,
                 ...
                 load_8bit, load_4bit, device, input_type, mel_size, attn_implementation="flash_attention_2"):
        ...
        self.tokenizer, self.model, self.image_processor, self.context_len = load_pretrained_model(
            model_path, model_base, model_name, load_8bit=load_8bit, load_4bit=load_4bit, device_map=self.device, attn_implementation=attn_implementation)
    ...
    def generate_stream(self, params):
        ...
        if video is not None and len(video) > 0:
            video = load_video(video, 8)
            video_tensor = self.image_processor.preprocess(video, return_tensors="pt")["pixel_values"].to(self.device, dtype=torch.float16)
            mm_args["images"] = [video_tensor]
            mm_args["modalities"] = ["video"]
        ...
        input_ids = tokenizer_image_speech_tokens(prompt, tokenizer, IMAGE_TOKEN_INDEX, SPEECH_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).to(self.device)
